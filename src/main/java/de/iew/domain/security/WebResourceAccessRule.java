...
public class WebResourceAccessRule extends AbstractModel {
    ...
    public static enum Discriminator {
         ...
         */
        HAS_AUTHORITY(HasAuthorityConfigAttribute.class),
        /**
         * The access rule is specified by a flag.
         */
        FLAG(FlagConfigAttribute.class);

        private Class configAttributeClass;

        private Discriminator(Class configAttributeClass) {
            this.configAttributeClass = configAttributeClass;
        }
         ...
         */
        public ConfigAttribute newConfigAttributeInstance(WebResourceAccessRule accessRule) throws ModelInstantiationException {
            try {
                return (ConfigAttribute) ConstructorUtils.invokeConstructor(this.configAttributeClass, accessRule);
            } catch (Exception e) {
                throw new ModelInstantiationException("Create config attribute constructor call failed on class " + this.configAttributeClass + ".", e);
            }
        }
    }
}
